...
export default {
  ...
  methods: {
    ...
    addToCart() {
      let cartItem = {
        productId: this.valId,
        qty: this.initalValue,
      };

      if (localStorage.getItem("freeCart")) {
        let oldCart = JSON.parse(localStorage.getItem("freeCart"));
        const found = oldCart.find((el) => this.valId == el.productId);
        if (!found) {
          oldCart.push(cartItem);

          localStorage.setItem("freeCart", JSON.stringify(oldCart));
        } else {
          found.qty = cartItem.qty;
          localStorage.setItem("freeCart", JSON.stringify(oldCart));
        }
      } else {
        localStorage.setItem("freeCart", JSON.stringify([cartItem]));
      }
